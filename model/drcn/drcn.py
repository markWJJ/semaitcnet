...
class DRCN(ModelTemplate):
    ...
    def build_encoder(self, sent_repres, input_lengths, *args, **kargs):
        ...
        word_emb = tf.nn.dropout(sent_repres, 1-dropout_rate)
    ...
    def build_interactor(self, sent1_emb, sent2_emb, 
        ...
        for i in range(num_lstm_layers):
            with tf.variable_scope(self.config.scope+"_densely_co_attentive_{}".format(i), 
                ...
                sent1_repres = tf.nn.dropout(sent1_repres, 1 - dropout_rate)
                sent2_repres = tf.nn.dropout(sent2_repres, 1 - dropout_rate)
        ...
        v = tf.concat([v_1_max, v_2_max, v_1_max + v_2_max, 
                        v_1_max*v_2_max,
                        v_1_max-v_2_max, tf.abs(v_1_max-v_2_max)],
                        axis=-1)
        v = tf.nn.dropout(v, 1 - dropout_rate)
        match_dim = match_dim * 6
